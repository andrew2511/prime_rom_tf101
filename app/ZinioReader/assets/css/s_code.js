...
s.linkInternalFilters="javascript:,zinio.com"
...
function s_doPlugins(s) {
	...
	if(!s.campaign)
		...
		s.campaign=s.getQueryParam('rf');

	

	if(!s.campaign)

		s.campaign=s.getQueryParam('cmpgn_var');

	/* click through quality */

	s.clickThruQuality('s_kwcpgn','event12','event13');
	...
	if(!s.eVar25)
		...
		s.eVar25=s.getQueryParam('irf');

	

	/* collect visit intent parameter */

	if(!s.eVar26)

		s.eVar26=s.getQueryParam('e26');
	...
	s.eVar34=s.getPreviousValue(s.pageName,'gpv_p5','');
...
s.visitorNamespace="zinio"

s.trackingServer="insight.zinio.com"

s.trackingServerSecure="insights.zinio.com"
